...
export function SiteDocumentsUploader() {
  ...
  // Get site documents from the context
  const siteDocuments = uploadedFiles['site_documents'] || [];
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of newFiles) {
        const result = await uploadFile('site_documents', file);
  ...
  return (
    <div className="border p-4 rounded-md">
      <Label htmlFor="site-documents" className="text-lg font-medium mb-2 block">
        Site Documents
      </Label>
      <p className="text-sm mb-4 text-gray-600">
        Upload site plans, surveys, existing conditions, or other site documentation.
      </p>
      
      <div className="flex flex-wrap gap-4 mb-4">
        {/* Display uploaded files from Supabase */}
        {siteDocuments.map((file) => {
