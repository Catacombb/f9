...
export function SupportingDocumentsUploader() {
  ...
  // Get supporting documents from the context
  const supportingDocuments = uploadedFiles['supporting_documents'] || [];
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of newFiles) {
        const result = await uploadFile('supporting_documents', file);
  ...
  return (
    <div className="border p-4 rounded-md">
      <Label htmlFor="supporting-documents" className="text-lg font-medium mb-2 block">
        Supporting Documents
      </Label>
      <p className="text-sm mb-4 text-gray-600">
        Upload additional files that may help us understand your project better.
      </p>
